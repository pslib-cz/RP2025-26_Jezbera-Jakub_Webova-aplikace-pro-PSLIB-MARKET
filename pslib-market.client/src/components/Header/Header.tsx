...
const PlusIcon = () => (
  <svg width="12" height="12" viewBox="0 0 12 12" fill="none" xmlns="http://www.w3.org/2000/svg">
    <path d="M1 6H11M6 1V11" stroke="white" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
  </svg>
    ...
    <div className={styles.headerShell}>
      <div className={styles.header}>
        <div className={styles.brand}>
          <img className={styles.logo} src='/logo.svg' alt='Logo aplikace PSLIB Market' width='60' height='50' />
          <p className={styles.title}>PSLIB <span className={styles.highlight}>MARKET</span></p>
        </div>
        <div className={styles.quickActions}>
          ...
          {auth.isAuthenticated ? (
            <Button icon={<SignOutIcon />} iconOnly onClick={() => { void handleLocalSignOut() }} variant='secondary' ariaLabel='Odhlásit se' />
          ) : (
            <Button icon={<SignInIcon />} iconOnly onClick={() => auth.signinRedirect()} ariaLabel='Přihlásit se' />
          )}
        </div>
        <div className={`${styles.searchRow} ${isMobileSearchOpen ? styles.searchRowOpen : ''}`.trim()}>
          <SearchBar />
        </div>
        <div className={styles.actionsRight}>
          <nav className={styles.nav} aria-label='Hlavní navigace'>
            <menu className={styles.menu}>
              <li>
                <Link className={`${styles.navLink} ${styles.navLinkSecondary}`.trim()} to='/moje-inzeraty'>
                  Moje inzeráty
                </Link>
              </li>
              <li>
                <Link className={`${styles.navLink} ${styles.navLinkPrimary}`.trim()} to='/vytvorit-inzerat'>
                  Vytvořit inzerát
