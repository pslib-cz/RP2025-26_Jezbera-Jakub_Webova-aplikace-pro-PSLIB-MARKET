...
import SortButtons, { type SortOption } from '../components/SortButtons/SortButtons'
...
const HomePage = () => {
  ...
  const [appliedFilters, setAppliedFilters] = useState<SidebarFilters>(createEmptyFilters)
...
const filtersIcon = (
    <svg width="20" height="20" viewBox="0 0 14 14" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
      <path d="M2 3H12L8.3 7.1V10.8L5.7 12V7.1L2 3Z" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
    </svg>
  );

  const sortIcon = (
    <svg width="18" height="18" viewBox="0 0 14 14" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
      <path d="M4 3V11M4 11L2.5 9.5M4 11L5.5 9.5M10 11V3M10 3L8.5 4.5M10 3L11.5 4.5" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
    </svg>
  );



  const normalizedCurrentUserEmail = String(auth.user?.profile?.email ?? '').trim().toLowerCase()

  const subjectOptions = [...new Set(books.flatMap((book) => (book.tags ?? []).filter(Boolean)))].sort((a, b) => a.localeCompare(b, 'cs'))
  const minAvailablePrice = books.length > 0 ? Math.min(...books.map((book) => book.price)) : 0
  const maxAvailablePrice = books.length > 0 ? Math.max(...books.map((book) => book.price)) : 1000
  ...
  const filterMatchedBooks = filteredBooks.filter((book) => {
    ...
    if (appliedFilters.subjects.length > 0) {
      const hasSelectedSubject = (book.tags ?? []).some((tag) => appliedFilters.subjects.includes(tag))
    ...
    if (appliedFilters.conditions.length > 0) {
      const numericCondition = typeof book.condition === 'string' ? parseInt(book.condition, 10) : book.condition
      if (typeof numericCondition !== 'number' || !appliedFilters.conditions.includes(numericCondition)) {
        return false
      }
    }
    ...
    if (appliedFilters.saleStatuses.length > 0) {
      ...
      const isReservedByMe = (book.reservations ?? []).some((reservation) =>
        reservation.reservedByUserEmail?.trim().toLowerCase() === normalizedCurrentUserEmail,
      )

      const matchesAnyStatus = appliedFilters.saleStatuses.some((selectedStatus) => {
  ...
  const getCreationTimestamp = (book: Book): number => {
    const parsedDate = book.createdAt ? new Date(book.createdAt).getTime() : Number.NaN
  ...
  const hasNoSearchResults = !isLoading && !loadError && books.length > 0 && filterMatchedBooks.length === 0
        ...
        <div className={styles.contentLayout}>
          ...
          <div className={styles.contentMain}>
            <div className={styles.mobileTopActions}>
              ...
              <Button text='Filtrovat' onClick={() => setIsMobileFilterOpen(true)} iconPosition='right' icon={filtersIcon}  variant='secondary' iconSize={18}/>

              <Button text='Řadit' onClick={() => setIsMobileSortOpen(true)} iconPosition='right' icon={sortIcon} iconSize={18} />
            </div>

            <div className={`${styles.toolbarRow} ${styles.desktopSortRow}`}>
              <SortButtons selectedSort={sortOption} onSortChange={setSortOption} />
            </div>

            {sortedBooks.length > 0 && (
              <div className={styles.bookGrid}>
                {sortedBooks.map((book) => {
                  const isReservedByCurrentUser = (book.reservations ?? []).some((reservation) =>
                    reservation.reservedByUserEmail?.trim().toLowerCase() === normalizedCurrentUserEmail,
                  )
                  const isOwnedByCurrentUser = book.ownerEmail?.trim().toLowerCase() === normalizedCurrentUserEmail
          ...
          <div className={styles.mobileSortSheet}>
            <div className={styles.mobileSortHeader}>
              ...
              <button
                ...
                aria-label='Zavřít řazení'
              >
                <svg width='18' height='18' viewBox='0 0 16 16' fill='none' xmlns='http://www.w3.org/2000/svg'>
                  <path d='M12.5 3.5L3.5 12.5' stroke='currentColor' strokeLinecap='round' strokeLinejoin='round' strokeWidth='2' />
                  <path d='M3.5 3.5L12.5 12.5' stroke='currentColor' strokeLinecap='round' strokeLinejoin='round' strokeWidth='2' />
                </svg>
              </button>
            </div>
              ...
              {mobileSortOptions.map((option) => {
                ...
                return (
                  <button
                    key={option.value}
                    type='button'
                    className={`${styles.mobileSortOption} ${isSelected ? styles.mobileSortOptionActive : ''}`.trim()}
                    onClick={() => {
                      setSortOption(option.value)
                      setIsMobileSortOpen(false)
                    }}
                  >
                    <span className={styles.mobileSortOptionIcon} aria-hidden='true'>{option.icon}</span>
